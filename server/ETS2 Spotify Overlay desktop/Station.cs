...
namespace ETS2_Spotify_Overlay
{
    static class Station
    {
        ...
        public static string _lastTrackName = "";
        public static string _lastArtists = "";
        ...
        public static System.Timers.Timer SpotifyTimer = new System.Timers.Timer();

        public static void SetSpotifyTrack(SpotifyManager.TrackInfo trackInfo)
        {
            try
            {
                if (trackInfo.TrackName == _lastTrackName && trackInfo.Artists == _lastArtists)
                {
                    return;
                }
                _lastTrackName = trackInfo.TrackName;
                _lastArtists = trackInfo.Artists;

                // Check if game is running and active (similar to radio stations)
                if (Main.ets2data == null || !Main.ets2data.SdkActive || Main.ets2data.Timestamp == 0)
                {
                    return;
                }
                ...
                if (Settings.Overlay)
                {
                    ...
                    string trackName = trackInfo.TrackName + " - " + trackInfo.Artists;
                    var stringSize = g.MeasureString(NowPlaying + " " + trackName, font);
                    var nowPlayingSize = g.MeasureString(NowPlaying + " ", font);
                    var nameSize = g.MeasureString(trackName, font);
                    ...
                    if (RTL)
                    {
                        g.DrawString(trackName, font, brush, topLeft);
                    ...
                    {
                        g.DrawString(trackName, font, brush, new PointF(topLeft.X + nowPlayingSize.Width, topLeft.Y));
                        g.DrawString(NowPlaying, font, Brushes.White, topLeft);
                    }

                    // Spotify is always connected, show full signal
                    g.DrawImage(Resources.spotify, 593, bmp.Height - 36, 32, 32);

                    // Draw album cover if available
                    if (!string.IsNullOrEmpty(trackInfo.AlbumCoverUrl))
                    {
                        try
                        {
                            using (var client = new WebClient())
                            {
                                var newPath = Directory.GetCurrentDirectory() + "\\tmp_spotify" + Path.GetExtension(new Uri(trackInfo.AlbumCoverUrl).AbsolutePath);
                                if (File.Exists(newPath))
                                {
                                    File.Delete(newPath);
                                }
                                client.DownloadFile(trackInfo.AlbumCoverUrl, newPath);
                                var logo = new Bitmap(newPath);
                                ...
                                g.DrawImage(logo, (256 / 2) - (logoWidth / 2) + 645, (bmp.Height / 2) - (logoHeight / 2), logoWidth, logoHeight);
                                logo.Dispose();
                            }
                        }
                        catch (Exception ex)
                        {
                            Log.Write("Spotify cover error: " + ex.ToString());
                        }
                    }
                    ...
                    SpotifyTimer = new System.Timers.Timer();
                    SpotifyTimer.Interval = 4000;
                    SpotifyTimer.Elapsed += (sender, args) =>
                    {
                        SpotifyTimer.Enabled = false;
                        SpotifyTimer.Stop();
                        overlay.Hidden = true;
                        ...
                        Log.Write("Hide spotify overlay");
                    };
